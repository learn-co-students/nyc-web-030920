...
- Discuss different encryption and hashing schemes, and `bcrypt` specifically
  - Do we want to ever store plaintext user passwords? NOOOOOOO!
...
- What's the difference between Authentication vs Authorization?
  - Authentication - Proviing identity (having a key)
  - Authorization - Allowing access (What dooes does the key unlock) 
...
#### Authorization
- can't create a Fantasy Creature unless logged in
- Can't delete a Fantasy Creature unless logged in
- Create a Post without having to select yourself
- Can only see the logged_in users show/prfile page
